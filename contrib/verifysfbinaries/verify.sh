...
###   This script attempts to download the signature file SHA256SUMS.asc from filbit.org
...
WORKINGDIR="/tmp/filbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://filbit.org/bin/"
VERSIONPREFIX="filbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'filbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Filbit Core binary release signing key installed?"
   fi
