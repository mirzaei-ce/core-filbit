...
{
    std::cout <<
        "This program outputs Filbit addresses and private keys from a wallet.dat file" << std::endl
        << std::endl
        << "Usage and options: "
        << std::endl
...
std::string WalletUtilityDB::getAddress(CDataStream ssKey)
{
    ...
    std::string strAddr = CFilbitAddress(id).ToString();
...
std::string WalletUtilityDB::getKey(CDataStream ssKey, CDataStream ssValue)
{
    ...
    if (key.Load(pkey, vchPubKey, true))
        strKey = CFilbitSecret(key).ToString();
...
std::string WalletUtilityDB::getCryptedKey(CDataStream ssKey, CDataStream ssValue, std::string masterPass)
{
    ...
    std::string strKey = CFilbitSecret(key).ToString();
    return strKey;
}
