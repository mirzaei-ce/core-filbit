Filbit Core version 0.11.0 is now available from:

  <https://filbit.org/bin/filbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Filbit Core or other software:
...
defaults to 0.00001. This will cause transactions with fewer FBT/kB fee to be
...
For example, add the following to `filbit.conf`:
...
related to the blockchain in the filbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Filbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Filbit Core will 
...
For an example script see [filbit-submittx](https://github.com/laanwj/filbit-submittx).
...
- #5334 `cf87536` libfilbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new filbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FilbitAmountField
- #5753 `0f44672` Add filbit logo to about screen
...
- #5809 `46bfbe7` Add filbit-cli man page
...
- #5849 `41259ca` contrib/init/filbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Filbit-Qt.app to Filbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for filbit-tx)
