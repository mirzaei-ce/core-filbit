...
*Setup instructions for a Gitian build of Filbit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Filbit
...
- [Building Filbit](#building-filbit)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-filbit-host/README.md).
...
Clone the git repositories for filbit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/filbit/filbit
```
...
in the filbit repository under 'Fetch and build inputs' to install sources which require
...
To build Filbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the filbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/filbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/filbit/filbit
...
For example:
```bash
URL=https://github.com/laanwj/filbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit filbit=${COMMIT} --url filbit=${URL} ../filbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit filbit=${COMMIT} --url filbit=${URL} ../filbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit filbit=${COMMIT} --url filbit=${URL} ../filbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the filbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../filbit/contrib/gitian-descriptors/*|sort|uniq )
...
FBTPATH=/some/root/path/filbit.git
SIGPATH=/some/root/path/filbit-detached-sigs.git

./bin/gbuild --url filbit=${FBTPATH},signature=${SIGPATH} ../filbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/filbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/filbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/filbit-osx-build.assert
```
...
[filbit/gitian.sigs](https://github.com/filbit/gitian.sigs/) repository, or if that's not possible create a pull
